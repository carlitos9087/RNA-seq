...
###########################################
# 0. Remocao de amostras indesejadas
###########################################
# Lista de amostras que serao removidas
samples_to_remove <- c("DC_Infected_Live_Parasites_3_SRR19400242", 
                       "DC_Infected_Fixed_Parasites_3_SRR19400239")

###########################################
# 1. Leitura e combinacao dos arquivos tabulares
###########################################
base_dir <- "Deseq2/PRJNA633893 - Leishmania/"
...
read_tabular_file <- function(file) {
  ...
  # Renomeia a coluna de contagem para o nome do arquivo (sem extensao)
...
# Converte os dados para uma matriz numerica
...
###########################################
# 2. Leitura dos dados fenotipicos (phenoData)
###########################################
# Supondo que o arquivo Excel contenha as colunas "id", "SampleName" e "Treatment"
phenoData <- read_excel("./Deseq2/PRJNA633893 - Leishmania/PRJNA633893.xlsx", col_names = TRUE)
lista <- phenoData$id
...
###########################################
# 3. QC - Deteccao de outliers com WGCNA
###########################################
...
###########################################
# 4. Funcoes para analise DESeq2, PCA, Sample-to-Sample Distances,
#    Dispersion estimates, Histogram of p-values e MA-plot
###########################################

# Funcao para Sample-to-Sample Distances com alta resolucao
...
# Funcao para o grafico de Dispersion Estimates com alta resolucao
...
# Funcao para gerar o Histograma de p-values com alta resolucao
...
# Funcao para detectar o organismo com base nos IDs
detect_organism <- function(entrez_ids) {
  ...
  if (any(entrez_ids %in% hs_ids)) {
    ...
    stop("Organismo nao reconhecido para os IDs fornecidos.")
  }
}

# Funcao para mapear IDs Entrez para simbolos de genes, ignorando erros
convert_entrez_to_symbol <- function(entrez_ids) {
  organism <- tryCatch(detect_organism(entrez_ids), error = function(e) NA)
  
  if (is.na(organism)) {
    warning("Nenhum organismo reconhecido. IDs serao ignorados.")
...
run_volcano_plot <- function(dds, output_file, pval_threshold = 0.05, log2fc_threshold = 1) {
  ...
  res_df$GeneID <- rownames(res_df)  # Mantem os IDs Entrez
  
  # Converte IDs Entrez para simbolos de genes
...
# phenoData
# Funcao DESeq2: analise e salvamento dos resultados
run_deseq_analysis <- function(data, phenoData, group1, group2, output_file) {
  # Cria o diretorio de saida, se nao existir
  dir.create(dirname(output_file), recursive = TRUE, showWarnings = FALSE)
  
  # Seleciona as amostras que pertencem aos grupos de interesse e que tambem estejam nos dados
  ...
  # Retorna o objeto DESeqDataSet para analises subsequentes
  return(dds)
}


# Funcao para criar os diagramas de Venn e salvar os resultados com a tabela contendo IDs e nomes de genes
run_venn_analysis <- function(dds1, dds2, comparison1, comparison2, output_prefix) {
  ...
  venn_up <- venn.diagram(
    ...
    alpha = 0.5,
    cex = 1.5,
    cat.cex = 0.8,     # Diminui o tamanho do texto dos circulos
    cat.pos = c(-30, 30),
  ...
  # Funcao auxiliar para gerar um data.frame com Entrez ID e Gene_Name
...
###########################################
# 5. Execucao das analises
###########################################
colnames(data)
intersect(phenoData$SampleName[phenoData$Treatment %in% c("Leish_CL", "Macrophages_Control")], colnames(data))
colnames(data)
phenoData


dds_8w <- run_deseq_analysis(data, phenoData, "Leish_CL", "Macrophages_Control",
                             file.path(results_dir, "DESeq2_Leish_CL_vs_Macrophages_Control.tabular"))


# dds_2w <- run_deseq_analysis(data, phenoData, "Uninfected", "Infected_2w",
#                              file.path(results_dir, "DESeq2_Uninfected_vs_Infected_2w.tabular"))
...
pca_24h <- run_pca_analysis(dds_8w, file.path(results_dir, "PCA_Leish_CL_vs_Macrophages_Control.jpeg"))
# pca_8h <- run_pca_analysis(dds_2w, file.path(results_dir, "PCA_LgyLRV1(-)_vs_LgyLRV1(+)_2w.jpeg"))



run_sample_distances(dds_8w, file.path(results_dir, "SampleDistances_Leish_CL_vs_Macrophages_Control.jpeg"))
# run_sample_distances(dds_2w, file.path(results_dir, "SampleDistances_Uninfected_vs_Infected_2w.jpeg"))


run_dispersion_plot(dds_8w, file.path(results_dir, "Dispersion_Leish_CL_vs_Macrophages_Control.jpeg"))
# run_dispersion_plot(dds_2w, file.path(results_dir, "Dispersion_Uninfected_vs_Infected_2w.jpeg"))


run_pval_histogram(dds_8w, file.path(results_dir, "Histogram_pvalues_Leish_CL_vs_Macrophages_Control.jpeg"))
# run_pval_histogram(dds_2w, file.path(results_dir, "Histogram_pvalues_Uninfected_vs_Infected_2w.jpeg"))

ma1 = run_ma_plot(dds_8w, file.path(results_dir, "MAplot_Leish_CL_vs_Macrophages_Control.jpeg"))
# ma2 = run_ma_plot(dds_2w, file.path(results_dir, "MAplot_Uninfected_vs_Infected_2w.jpeg"))

head(ma1$data)

# Executa e salva os Volcano Plots
run_volcano_plot(dds_8w, file.path(results_dir, "Volcano_Leish_CL_vs_Macrophages_Control.jpeg"))
# run_volcano_plot(dds_2w, file.path(results_dir, "Volcano_Uninfected_vs_Infected_2w.jpeg"))
...
run_venn_analysis(dds_8w, dds_2w,
                  "Uninfected_vs_Infected_8w", "Uninfected_vs_Infected_2w",
...
for(sheet in sheet_names) {
  ...
  if(!("Entrez_ID" %in% colnames(gene_df))) {
    stop("A coluna 'Entrez_ID' nao foi encontrada na aba ", sheet)
  }
  ...
  # Detecta o organismo usando a funcao detect_organism
  ...
  # Se nao houver genes mapeados, pula a aba
  ...
  # Gera e salva um dotplot (como PDF) para visualizacao
...
# Funcao para selecionar o banco de dados correto
...
# Funcao principal para analise de enriquecimento
...
# # Exemplo de uso:
# # Supondo que 'dds_nf_ilp' seja o objeto DESeq2 resultante da comparacao desejada.
...
run_deseq_up_down_analysis <- function(dds, up_threshold, down_threshold, padj_cutoff, output_file) {
  ...
  # Adiciona os simbolos aos data frames
...
up_down_enrich_results <- run_deseq_up_down_enrichment(dds = dds_8w,
                                                       up_threshold = 1,
                                                       down_threshold = -1,
                                                       padj_cutoff = 0.05,
                                                       ont = "BP",
                                                       ...
                                                       pvalueCutoff = 0.05,
                                                       qvalueCutoff = 0.2,
                                                       output_file_prefix = file.path(results_dir, "DESeq2_UpDown_Leish_CL_vs_Macrophages_Control"))

# up_down_enrich_results <- run_deseq_up_down_enrichment(dds = dds_2w,
#                                                        up_threshold = 1,
#                                                        down_threshold = -1,
#                                                        padj_cutoff = 0.05,
#                                                        ont = "BP",
...
#                                                        pvalueCutoff = 0.05,
#                                                        qvalueCutoff = 0.2,
#                                                        output_file_prefix = file.path(results_dir, "DESeq2_UpDown_2w"))
