...
run_deseq_up_down_enrichment <- function(dds,
  ...
  # (1) Extrai DEGs e mantem em memoria
...
dds_8w
up_down_enrich_Leish_CL_vs_Macrophages_Control <- run_deseq_up_down_enrichment(
  dds             = dds_8w,
  up_threshold    = 1,
  down_threshold  = -1,
  padj_cutoff     = 0.05,
  ...
  pvalueCutoff    = 0.05,
  qvalueCutoff    = 0.2,
  output_prefix   = "./Deseq2/PRJNA633893 - Leishmania/results/DESeq2_UpDown_Leish_CL_vs_Macrophages_Control"
)
...
resultados <- list(
  dds_8w = up_down_enrich_Leish_CL_vs_Macrophages_Control
)

# Diretorios de saida correspondentes
diretorios_saida <- list(
  dds_8w = "./Deseq2/PRJNA633893 - Leishmania/results/Enrichment_AnalysisLeish_CL_vs_Macrophages_Control/"
...
walk(names(resultados), function(nome) {
  ...
  # Define os caminhos dos arquivos de saida
  ...
  # (C) Uniao (termos ??? targets), sem duplicar entradas por category+ID
