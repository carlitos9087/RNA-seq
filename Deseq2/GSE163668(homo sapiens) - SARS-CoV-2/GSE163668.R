...
phenoData <- read_excel("./Deseq2/GSE163668(homo sapiens) - SARS-CoV-2/GSE163668.xlsx", col_names = TRUE)
lista <- phenoData$id
...
intersect(phenoData$SampleName[phenoData$Treatment %in% c("control", "phenotype_mild_moderate")], colnames(data))
colnames(data)
phenoData
...
dds_control_vs_mild_moderate <- run_deseq_analysis(
  data, phenoData, 
  "control", "phenotype_mild_moderate", 
  file.path(results_dir, "DESeq2_control_vs_mild_moderate.tabular")
)

dds_control_vs_severe <- run_deseq_analysis(
  data, phenoData, 
  "control", "phenotype_severe", 
  file.path(results_dir, "DESeq2_control_vs_severe.tabular")
)

# Análises de PCA
pca_control_vs_mild_moderate <- run_pca_analysis(
  dds_control_vs_mild_moderate, 
  file.path(results_dir, "PCA_control_vs_mild_moderate.jpeg")
)

pca_control_vs_severe <- run_pca_analysis(
  dds_control_vs_severe, 
  file.path(results_dir, "PCA_control_vs_severe.jpeg")
)

# Distâncias entre amostras
run_sample_distances(
  dds_control_vs_mild_moderate, 
  file.path(results_dir, "SampleDistances_control_vs_mild_moderate.jpeg")
)

run_sample_distances(
  dds_control_vs_severe, 
  file.path(results_dir, "SampleDistances_control_vs_severe.jpeg")
)

# Gráficos de dispersão
run_dispersion_plot(
  dds_control_vs_mild_moderate, 
  file.path(results_dir, "Dispersion_control_vs_mild_moderate.jpeg")
)

run_dispersion_plot(
  dds_control_vs_severe, 
  file.path(results_dir, "Dispersion_control_vs_severe.jpeg")
)

# Histogramas de p-values
run_pval_histogram(
  dds_control_vs_mild_moderate, 
  file.path(results_dir, "Histogram_pvalues_control_vs_mild_moderate.jpeg")
)

run_pval_histogram(
  dds_control_vs_severe, 
  file.path(results_dir, "Histogram_pvalues_control_vs_severe.jpeg")
)

# MA Plots
ma_control_vs_mild_moderate <- run_ma_plot(
  dds_control_vs_mild_moderate, 
  file.path(results_dir, "MAplot_control_vs_mild_moderate.jpeg")
)

ma_control_vs_severe <- run_ma_plot(
  dds_control_vs_severe, 
  file.path(results_dir, "MAplot_control_vs_severe.jpeg")
)

# Volcano Plots
run_volcano_plot(
  dds_control_vs_mild_moderate, 
  file.path(results_dir, "Volcano_control_vs_mild_moderate.jpeg")
)

run_volcano_plot(
  dds_control_vs_severe, 
  file.path(results_dir, "Volcano_control_vs_severe.jpeg")
)
...
run_venn_analysis(
  dds_control_vs_mild_moderate, 
  dds_control_vs_severe,
  "control_vs_mild_moderate", 
  "control_vs_severe",
  file.path(results_dir, "Venn_control_vs_mild_moderate_vs_severe")
)
