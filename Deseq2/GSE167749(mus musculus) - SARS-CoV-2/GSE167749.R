...
###########################################
base_dir <- "Deseq2/GSE167749(mus musculus) - SARS-CoV-2/"
...
phenoData <- read_excel("./Deseq2/GSE167749(mus musculus) - SARS-CoV-2/SRP377060.xlsx", col_names = TRUE)
lista <- phenoData$id
...
intersect(phenoData$SampleName[phenoData$Treatment %in% c("healthy", "MCoV_infected")], colnames(data))
colnames(data)
phenoData

# # Executa DESeq2 para as comparações desejadas
# Executa DESeq2 para a comparação healthy vs MCoV_infected
dds_Healthy_vs_MCoV <- run_deseq_analysis(
  data, phenoData, 
  "healthy",          # Grupo controle
  "MCoV_infected",    # Grupo experimental (ajuste o nome se necessário para corresponder exatamente ao phenoData)
  file.path(results_dir, "DESeq2_Healthy_vs_MCoV_Infected.tabular")
)

# Análises de PCA
pca_Healthy_vs_MCoV <- run_pca_analysis(
  dds_Healthy_vs_MCoV, 
  file.path(results_dir, "PCA_Healthy_vs_MCoV_Infected.jpeg")
)

# Distâncias entre amostras
run_sample_distances(
  dds_Healthy_vs_MCoV, 
  file.path(results_dir, "SampleDistances_Healthy_vs_MCoV_Infected.jpeg")
)

# Gráficos de dispersão
run_dispersion_plot(
  dds_Healthy_vs_MCoV, 
  file.path(results_dir, "Dispersion_Healthy_vs_MCoV_Infected.jpeg")
)

# Histogramas de p-values
run_pval_histogram(
  dds_Healthy_vs_MCoV, 
  file.path(results_dir, "Histogram_pvalues_Healthy_vs_MCoV_Infected.jpeg")
)

# MA Plot
ma_Healthy_vs_MCoV <- run_ma_plot(
  dds_Healthy_vs_MCoV, 
  file.path(results_dir, "MAplot_Healthy_vs_MCoV_Infected.jpeg")
)

# Volcano Plot
run_volcano_plot(
  dds_Healthy_vs_MCoV, 
  file.path(results_dir, "Volcano_Healthy_vs_MCoV_Infected.jpeg")
)
...
up_down_enrich_dds_Healthy_vs_MCoVh <- run_deseq_up_down_enrichment(
  dds             = dds_Healthy_vs_MCoV,
  up_threshold    = 1,
  down_threshold  = -1,
  padj_cutoff     = 0.05,
  ontologies      = c("BP","MF"),
  ...
  pvalueCutoff    = 0.05,
  qvalueCutoff    = 0.2,
  output_prefix = file.path(results_dir, "DESeq2_UpDown_Healthy_vs_MCoV_Infected")
...
resultados <- list(
  dds_Healthy_vs_MCoV = up_down_enrich_dds_Healthy_vs_MCoVh
)

# Diretórios de saída correspondentes
diretorios_saida <- list(
  dds_Healthy_vs_MCoV = file.path(results_dir, "Enrichment_Analysis_UpDown_Healthy_vs_MCoV_Infected")
