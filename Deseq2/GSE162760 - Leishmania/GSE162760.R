...
###########################################
base_dir <- "Deseq2/GSE162760 - Leishmania/"
...
phenoData <- read_excel("./Deseq2/GSE162760 - Leishmania/GSE162760.xlsx", col_names = TRUE)
lista <- phenoData$id
...
intersect(phenoData$SampleName[phenoData$Treatment %in% c("Control_Non-infected", "Infected_Lbr")], colnames(data))
colnames(data)
phenoData
...
dds_Control_NonInfected_vs_InfectedLbr <- run_deseq_analysis(
  data, phenoData, 
  "Control_Non-infected",    # Grupo controle
  "Infected_Lbr",            # Grupo experimental
  file.path(results_dir, "DESeq2_Control_Non-infected_vs_Infected_Lbr.tabular")
)

# Análises de PCA
pca_Control_vs_Infected <- run_pca_analysis(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "PCA_Control_Non-infected_vs_Infected_Lbr.jpeg")
)

# Distâncias entre amostras
run_sample_distances(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "SampleDistances_Control_Non-infected_vs_Infected_Lbr.jpeg")
)

# Gráficos de dispersão
run_dispersion_plot(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "Dispersion_Control_Non-infected_vs_Infected_Lbr.jpeg")
)

# Histogramas de p-values
run_pval_histogram(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "Histogram_pvalues_Control_Non-infected_vs_Infected_Lbr.jpeg")
)

# MA Plot
ma_Control_vs_Infected <- run_ma_plot(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "MAplot_Control_Non-infected_vs_Infected_Lbr.jpeg")
)

# Volcano Plot
run_volcano_plot(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "Volcano_Control_Non-infected_vs_Infected_Lbr.jpeg")
)
...
up_down_enrich_dds_Control_NonInfected_vs_InfectedLbr <- run_deseq_up_down_enrichment(
  ...
  pvalueCutoff    = 0.05,
  qvalueCutoff    = 0.2,
  output_prefix = file.path(results_dir, "DESeq2_UpDown_Control_NonInfected_vs_InfectedLbr")
...
resultados <- list(
  dds_Control_NonInfected_vs_InfectedLbr = up_down_enrich_dds_Control_NonInfected_vs_InfectedLbr
)

# Diretórios de saída correspondentes
diretorios_saida <- list(
  dds_Control_NonInfected_vs_InfectedLbr = file.path(results_dir, "Enrichment_Analysis_UpDown_Control_NonInfected_vs_InfectedLbr")
