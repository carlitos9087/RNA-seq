...
###########################################
base_dir <- "Deseq2/PRJNA838552 - Leishmania/"
...
phenoData <- read_excel("./Deseq2/PRJNA838552 - Leishmania/PRJNA838552.xlsx", col_names = TRUE)
lista <- phenoData$id
...
# Extração dos fatores de forma "sanitizada"
phenoData$Time <- factor(ifelse(grepl("_24h", phenoData$Treatment), "24h", "8h"))
phenoData$Virus <- factor(ifelse(grepl("LgyLRV1\\(\\+\\)", phenoData$Treatment), "LRV1pos", "LRV1neg"))
...
##############
run_deseq_analysis <- function(data, phenoData, time_point, group1, group2, output_file) {
  # Cria o diretório de saída, se não existir
  dir.create(dirname(output_file), recursive = TRUE, showWarnings = FALSE)
  
  # Filtra as amostras para o tempo desejado (por exemplo, "24h")
  phenoData_sub <- subset(phenoData, Time == time_point)
  
  # Seleciona as amostras que pertencem aos grupos de interesse no fator Virus
  valid_samples <- intersect(phenoData_sub$SampleName[phenoData_sub$Virus %in% c(group1, group2)], colnames(data))
  if(length(valid_samples) == 0){
    stop("Nenhuma amostra encontrada para os grupos especificados.")
  }
  ...
  # Subconjunto dos dados de contagem e dos metadados
  counts <- data[, valid_samples, drop = FALSE]
  pheno <- phenoData_sub[phenoData_sub$SampleName %in% valid_samples, ]
  
  # Cria o objeto DESeqDataSet usando apenas o fator Virus
  dds <- DESeqDataSetFromMatrix(countData = counts,
                                colData = pheno,
                                design = ~ Virus)
  ...
  # Extrai os resultados do contraste especificado (por exemplo, "LRV1pos" vs "LRV1neg")
  res <- results(dds, contrast = c("Virus", group2, group1))
...
dds_result_24 <- run_deseq_analysis(data, phenoData, time_point = "24h",
                                 group1 = "LRV1neg", group2 = "LRV1pos",
                                 output_file = file.path(results_dir, "contrast_24h.tabular"))

dds_result_8h <- run_deseq_analysis(data, phenoData, time_point = "8h",
                                    group1 = "LRV1neg", group2 = "LRV1pos",
                                    output_file = file.path(results_dir, "contrast_8h.tabular"))
...
run_venn_analysis <- function(dds1, dds2, comparison1, comparison2, output_prefix) {
  ...
  venn_up <- venn.diagram(
    ...
    alpha = 0.5,
    cex = 1.5,
    cat.cex = 0.7,     # Diminui o tamanho do texto dos círculos
    cat.pos = c(-30, 30),
...
#                                  file.path(dds_result_24, "DESeq2_LgyLRV1(-)_vs_LgyLRV1(+).tabular"))
# 
...
# pca_nf_ilp <- run_pca_analysis(res, file.path(dds_result_24, "PCA_LgyLRV1(-)_vs_LgyLRV1(+)_24h.jpeg"))
pca_24h <- run_pca_analysis(dds_result_24, file.path(results_dir, "PCA_LgyLRV1(-)_vs_LgyLRV1(+)_24h.jpeg"))
pca_8h <- run_pca_analysis(dds_result_8h, file.path(results_dir, "PCA_LgyLRV1(-)_vs_LgyLRV1(+)_8h.jpeg"))



run_sample_distances(dds_result_24, file.path(results_dir, "SampleDistances_LgyLRV1(-)_vs_LgyLRV1(+)_24h.jpeg"))
run_sample_distances(dds_result_8h, file.path(results_dir, "SampleDistances__LgyLRV1(-)_vs_LgyLRV1(+)_8h.jpeg"))


run_dispersion_plot(dds_result_24, file.path(results_dir, "Dispersion_LgyLRV1(-)_vs_LgyLRV1(+)_24h.jpeg"))
run_dispersion_plot(dds_result_8h, file.path(results_dir, "Dispersion_LgyLRV1(-)_vs_LgyLRV1(+)_8h.jpeg"))


run_pval_histogram(dds_result_24, file.path(results_dir, "Histogram_pvalues_LgyLRV1(-)_vs_LgyLRV1(+)_24h.jpeg"))
run_pval_histogram(dds_result_8h, file.path(results_dir, "Histogram_pvalues_LgyLRV1(-)_vs_LgyLRV1(+)_8h.jpeg"))

ma1 = run_ma_plot(dds_result_24, file.path(results_dir, "MAplot_LgyLRV1(-)_vs_LgyLRV1(+)_24h.jpeg"))
ma2 = run_ma_plot(dds_result_8h, file.path(results_dir, "MAplot_LgyLRV1(-)_vs_LgyLRV1(+)_8h.jpeg"))

head(ma1$data)

# Executa e salva os Volcano Plots
run_volcano_plot(dds_result_24, file.path(results_dir, "Volcano_LgyLRV1(-)_vs_LgyLRV1(+)_24h.jpeg"))
run_volcano_plot(dds_result_8h, file.path(results_dir, "Volcano_LgyLRV1(-)_vs_LgyLRV1(+)_8h.jpeg"))
...
run_venn_analysis(dds_result_24, dds_result_8h,
                  "LgyLRV1(-)_vs_LgyLRV1(+)_24h", "LgyLRV1(-)_vs_LgyLRV1(+)_8h",
...
input_file <- "./Deseq2/PRJNA838552 - Leishmania/results/Venn_Analysis_VennData.xlsx"
...
up_down_enrich_results_dds_result_24 <- run_deseq_up_down_enrichment(
  dds             = dds_result_24,
  up_threshold    = 1,
  down_threshold  = -1,
  padj_cutoff     = 0.05,
  ...
  pvalueCutoff    = 0.05,
  qvalueCutoff    = 0.2,
  output_prefix   = "./Deseq2/PRJNA838552 - Leishmania/results/DESeq2_UpDown_dds_result_24"
)

up_down_enrich_results_dds_result_8h <- run_deseq_up_down_enrichment(
  dds             = dds_result_8h,
  up_threshold    = 1,
  down_threshold  = -1,
  padj_cutoff     = 0.05,
  ...
  pvalueCutoff    = 0.05,
  qvalueCutoff    = 0.2,
  output_prefix   = "./Deseq2/PRJNA838552 - Leishmania/results/DESeq2_UpDown_dds_result_8h"
)
...
# }
# 
# up_down_enrich_results <- run_deseq_up_down_enrichment(dds = dds_result_24,
#                                                        up_threshold = 1,
#                                                        down_threshold = -1,
#                                                        padj_cutoff = 0.05,
#                                                        ont = "BP",
...
#                                                        pvalueCutoff = 0.05,
#                                                        qvalueCutoff = 0.2,
#                                                        output_file_prefix = file.path(results_dir, "DESeq2_UpDown_24h"))
# 
# up_down_enrich_results <- run_deseq_up_down_enrichment(dds = dds_result_8h,
#                                                        up_threshold = 1,
#                                                        down_threshold = -1,
#                                                        padj_cutoff = 0.05,
#                                                        ont = "BP",
...
#                                                        pvalueCutoff = 0.05,
#                                                        qvalueCutoff = 0.2,
#                                                        output_file_prefix = file.path(results_dir, "DESeq2_UpDown_8h"))


###############################################################
###############################################################

up_down_enrich_results_dds_result_24
up_down_enrich_results_dds_result_8h

# Listas de resultados de enriquecimento
resultados <- list(
  dds_result_24 = up_down_enrich_results_dds_result_24,
  dds_result_8h = up_down_enrich_results_dds_result_8h
)

# Diretórios de saída correspondentes
diretorios_saida <- list(
  dds_result_24 = "./Deseq2/PRJNA838552 - Leishmania/results/dds_result_24/",
  dds_result_8h   = "./Deseq2/PRJNA838552 - Leishmania/results/dds_result_8h/"
)
