...
###########################################
base_dir <- "Deseq2/GSE156072(homo sapiens)- HIV/"
...
phenoData <- read_excel("./Deseq2/GSE156072(homo sapiens)- HIV/GSE156072.xlsx", col_names = TRUE)
lista <- phenoData$id
...
intersect(phenoData$SampleName[phenoData$Treatment %in% c("Uninfected", "YU2_HIV_infected")], colnames(data))
colnames(data)
phenoData



# Executa DESeq2 para a comparação Control_Non-infected vs Infected_Lbr
dds_Control_NonInfected_vs_InfectedLbr <- run_deseq_analysis(
  data, phenoData, 
  "Uninfected",    # Grupo controle
  "YU2_HIV_infected",            # Grupo experimental
  file.path(results_dir, "DESeq2_Uninfected_vs_YU2_HIV_infected.tabular")
)

# Análises de PCA
pca_Control_vs_Infected <- run_pca_analysis(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "PCA_Uninfected_vs_YU2_HIV_infected.jpeg")
)

# Distâncias entre amostras
run_sample_distances(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "SampleDistances_Uninfected_vs_YU2_HIV_infected.jpeg")
)

# Gráficos de dispersão
run_dispersion_plot(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "Dispersion_Uninfected_vs_YU2_HIV_infected.jpeg")
)

# Histogramas de p-values
run_pval_histogram(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "Histogram_pvalues_Uninfected_vs_YU2_HIV_infected.jpeg")
)

# MA Plot
ma_Control_vs_Infected <- run_ma_plot(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "MAplot_Uninfected_vs_YU2_HIV_infected.jpeg")
)

# Volcano Plot
run_volcano_plot(
  dds_Control_NonInfected_vs_InfectedLbr, 
  file.path(results_dir, "Volcano_Uninfected_vs_YU2_HIV_infected.jpeg")
)
...
up_down_enrich_results_Uninfected_vs_YU2_HIV_infected <- run_deseq_up_down_enrichment(
  dds             = dds_Control_NonInfected_vs_InfectedLbr,
  up_threshold    = 1,
  down_threshold  = -1,
  padj_cutoff     = 0.05,
  ontologies      = c("BP","MF"),
  ...
  pvalueCutoff    = 0.05,
  qvalueCutoff    = 0.2,
  output_prefix = file.path(results_dir, "DESeq2_UpDown_Uninfected_vs_YU2_HIV_infected")
  
)
up_down_enrich_results_Uninfected_vs_YU2_HIV_infected
results_dir

resultados <- list(
  dds_Control_NonInfected_vs_InfectedLbr = up_down_enrich_results_Uninfected_vs_YU2_HIV_infected
)

# Diretórios de saída correspondentes
diretorios_saida <- list(
  dds_Control_NonInfected_vs_InfectedLbr = file.path(results_dir, "Enrichment_Analysis_UpDown_Uninfected_vs_YU2_HIV_infected")
