...
###########################################
base_dir <- "Deseq2/GSE103666(homo sapiens) - HIV/"
...
phenoData <- read_excel("Deseq2/GSE103666(homo sapiens) - HIV/GSE103666.xlsx", col_names = TRUE)
lista <- phenoData$id
...
# Executa DESeq2 para a comparação Uninfected vs Infected
dds_Uninfected_vs_Infected <- run_deseq_analysis(
  data, phenoData, 
  "Uninfected",       # Grupo controle
  "Infected",         # Grupo experimental
  file.path(results_dir, "DESeq2_Uninfected_vs_Infected.tabular")
)

# Análises de PCA
pca_Uninfected_vs_Infected <- run_pca_analysis(
  dds_Uninfected_vs_Infected, 
  file.path(results_dir, "PCA_Uninfected_vs_Infected.jpeg")
)

# Distâncias entre amostras
run_sample_distances(
  dds_Uninfected_vs_Infected, 
  file.path(results_dir, "SampleDistances_Uninfected_vs_Infected.jpeg")
)

# Gráficos de dispersão
run_dispersion_plot(
  dds_Uninfected_vs_Infected, 
  file.path(results_dir, "Dispersion_Uninfected_vs_Infected.jpeg")
)

# Histogramas de p-values
run_pval_histogram(
  dds_Uninfected_vs_Infected, 
  file.path(results_dir, "Histogram_pvalues_Uninfected_vs_Infected.jpeg")
)

# MA Plot
ma_Uninfected_vs_Infected <- run_ma_plot(
  dds_Uninfected_vs_Infected, 
  file.path(results_dir, "MAplot_Uninfected_vs_Infected.jpeg")
)

# Volcano Plot
run_volcano_plot(
  dds_Uninfected_vs_Infected, 
  file.path(results_dir, "Volcano_Uninfected_vs_Infected.jpeg")
)
...
up_down_enrich_results <- run_deseq_up_down_enrichment(
  dds = dds_Uninfected_vs_Infected,
  up_threshold = 1,
  down_threshold = -1,
  padj_cutoff = 0.05,
  ont = "BP",
  ...
  pvalueCutoff = 0.05,
  qvalueCutoff = 0.2,
  output_file_prefix = file.path(results_dir, "DESeq2_UpDown_Uninfected_vs_Infected")
)
