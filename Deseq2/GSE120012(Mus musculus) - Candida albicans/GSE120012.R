...
###########################################
base_dir <- "Deseq2/GSE120012(Mus musculus) - Candida albicans/"
...
phenoData <- read_excel("Deseq2/GSE120012(Mus musculus) - Candida albicans/GSE120012.xlsx", col_names = TRUE)
lista <- phenoData$id
...
# Executa DESeq2 para a comparação naive vs CAWS_challenged
dds_naive_vs_CAWS <- run_deseq_analysis(
  data, phenoData, 
  "naive",                 # Grupo controle
  "CAWS_challenged",       # Grupo experimental
  file.path(results_dir, "DESeq2_naive_vs_CAWS_challenged.tabular")
)

# Análises de PCA
pca_naive_vs_CAWS <- run_pca_analysis(
  dds_naive_vs_CAWS, 
  file.path(results_dir, "PCA_naive_vs_CAWS_challenged.jpeg")
)

# Distâncias entre amostras
run_sample_distances(
  dds_naive_vs_CAWS, 
  file.path(results_dir, "SampleDistances_naive_vs_CAWS_challenged.jpeg")
)

# Gráficos de dispersão
run_dispersion_plot(
  dds_naive_vs_CAWS, 
  file.path(results_dir, "Dispersion_naive_vs_CAWS_challenged.jpeg")
)

# Histogramas de p-values
run_pval_histogram(
  dds_naive_vs_CAWS, 
  file.path(results_dir, "Histogram_pvalues_naive_vs_CAWS_challenged.jpeg")
)

# MA Plot
ma_naive_vs_CAWS <- run_ma_plot(
  dds_naive_vs_CAWS, 
  file.path(results_dir, "MAplot_naive_vs_CAWS_challenged.jpeg")
)

# Volcano Plot
run_volcano_plot(
  dds_naive_vs_CAWS, 
  file.path(results_dir, "Volcano_naive_vs_CAWS_challenged.jpeg")
)
...
up_down_enrich_results <- run_deseq_up_down_enrichment(
  ...
  pvalueCutoff = 0.05,
  qvalueCutoff = 0.2,
  output_file_prefix = file.path(results_dir, "DESeq2_UpDown_naive_vs_CAWS_challenged")
)
