...
###########################################
base_dir <- "Deseq2/PRJNA707548 - M tuberculosis/"
...
phenoData <- read_excel("./Deseq2/PRJNA707548 - M tuberculosis/PRJNA707548.xlsx", col_names = TRUE)
lista <- phenoData$id
...
intersect(phenoData$SampleName[phenoData$Treatment %in% c("Uninfected", "Infected_8w")], colnames(data))
colnames(data)
phenoData


# Executa DESeq2 para as comparações desejadas
dds_8w <- run_deseq_analysis(data, phenoData, "Uninfected", "Infected_8w",
                                 file.path(results_dir, "DESeq2_Uninfected_vs_Infected_8w.tabular"))
...
pca_24h <- run_pca_analysis(dds_8w, file.path(results_dir, "PCA_Uninfected_vs_Infected_8w.jpeg"))
pca_8h <- run_pca_analysis(dds_2w, file.path(results_dir, "PCA_LgyLRV1(-)_vs_LgyLRV1(+)_2w.jpeg"))



run_sample_distances(dds_8w, file.path(results_dir, "SampleDistances_Uninfected_vs_Infected_8w.jpeg"))
run_sample_distances(dds_2w, file.path(results_dir, "SampleDistances_Uninfected_vs_Infected_2w.jpeg"))


run_dispersion_plot(dds_8w, file.path(results_dir, "Dispersion_Uninfected_vs_Infected_8w.jpeg"))
run_dispersion_plot(dds_2w, file.path(results_dir, "Dispersion_Uninfected_vs_Infected_2w.jpeg"))


run_pval_histogram(dds_8w, file.path(results_dir, "Histogram_pvalues_Uninfected_vs_Infected_8w.jpeg"))
run_pval_histogram(dds_2w, file.path(results_dir, "Histogram_pvalues_Uninfected_vs_Infected_2w.jpeg"))

ma1 = run_ma_plot(dds_8w, file.path(results_dir, "MAplot_Uninfected_vs_Infected_8w.jpeg"))
ma2 = run_ma_plot(dds_2w, file.path(results_dir, "MAplot_Uninfected_vs_Infected_2w.jpeg"))

head(ma1$data)

# Executa e salva os Volcano Plots
run_volcano_plot(dds_8w, file.path(results_dir, "Volcano_Uninfected_vs_Infected_8w.jpeg"))
...
input_file <- "./Deseq2/PRJNA707548 - M tuberculosis/results/Venn_Analysis_VennData.xlsx"
...
up_down_enrich_results_dds_8w <- run_deseq_up_down_enrichment(
  dds             = dds_8w,
  up_threshold    = 1,
  down_threshold  = -1,
  padj_cutoff     = 0.05,
  ...
  pvalueCutoff    = 0.05,
  qvalueCutoff    = 0.2,
  output_prefix   = "./Deseq2/PRJNA707548 - M tuberculosis/results/DESeq2_UpDown_8w"
)

up_down_enrich_results_dds_2w <- run_deseq_up_down_enrichment(
  dds             = dds_2w,
  up_threshold    = 1,
  down_threshold  = -1,
  padj_cutoff     = 0.05,
  ...
  pvalueCutoff    = 0.05,
  qvalueCutoff    = 0.2,
  output_prefix   = "./Deseq2/PRJNA707548 - M tuberculosis/results/DESeq2_UpDown_2w"
)
...
resultados <- list(
  dds_2w = up_down_enrich_results_dds_2w,
  dds_8w = up_down_enrich_results_dds_8w
)

# Diretórios de saída correspondentes
diretorios_saida <- list(
  dds_2w = "./Deseq2/PRJNA707548 - M tuberculosis/results/dds_2w/",
  dds_8w   = "./Deseq2/PRJNA707548 - M tuberculosis/results/dds_8w/"
)
