...
tabular_dir  <- "/Users/carlitos/Desktop/experimentos/exps fasta/fastas - Copia/Candida albicans/GSE120012(Mus musculus)//"
...
colnames(data)
# dados da amostra
phenoData  <-  read_excel("/Users/carlitos/Desktop/GSE120012.xlsx", col_names = TRUE)
lista  <- phenoData$id
...
cores_interesse <- c("brown", "purple")
...
write.csv(as.data.frame(gene.signf.corr.pvals), file = "/Users/carlitos/Desktop/GSE120012 gene.signf.corr.pvals.csv", row.names = TRUE)
...
modules_of_interest = c("purple", "brown")
...
ego <- enrichGO(gene          = black_genes,
                OrgDb         = org.Mm.eg.db,   # Substitua pelo banco de dados apropriado
...
ekegg <- enrichKEGG(gene         = black_genes,
                    organism     = 'mmu',       # Organismo apropriado para Mus musculus
                    pAdjustMethod = "BH",
                    pvalueCutoff = 0.05,
                    qvalueCutoff = 0.05)
...
#####################################################################################'
# Extrair genes do módulo preto
black_genes <- names(bwnet$colors[bwnet$colors == "purple"])
black_genes
ego <- enrichGO(gene          = black_genes,
                OrgDb         = org.Mm.eg.db,   # Substitua pelo banco de dados apropriado
                keyType       = "ENTREZID",       # Substitua por "ENTREZID" se usar IDs Entrez
