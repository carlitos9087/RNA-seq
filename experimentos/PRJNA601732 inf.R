...
tabular_dir  <- "/Users/carlitos/Desktop/experimentos/exps fasta/fastas/Leishmania/PRJNA601732 - inf/"
...
phenoData  <-  read_excel("/Users/carlitos/Desktop/aaaaaaaaaaaaaaaaaaaaaaaaaaa - inf - Copia.xlsx ", col_names = TRUE)
lista  <- phenoData$id
...
head(colData,15)
traits <- colData %>% 
  mutate(disease_state_bin = ifelse(grepl('Linfantum', Treatment), 1, 0)) #%>% select(2)
...
colData$severity <- factor(colData$Treatment, levels = c(
  "Infected_Mouse_Linfantum","Uninfected_Mouse"))
